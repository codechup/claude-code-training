...
depends_on: [P24]
owned_paths:
  - content/en/l4-master/m16-orchestration/**
  - content/tr/l4-master/m16-orchestration/**
  - content/_shared/transcripts/m16-orchestration/**
shared_paths:
...
Author every lesson of Orchestration (`m16-orchestration`), the 3-lesson next module of Master (l4-master), in English: the reader designs and runs a small multi-agent pipeline of their own, using the same coordination ideas this project itself runs on (see m18-multi-session). Each lesson follows the fixed template (D006): Objectives + prerequisites, When NOT to use, Concept, Hands-on lab, Anti-patterns, Quiz, Sources. Every command shown was actually run against the lab repo and every claim is backed by real output (D093) — this plan produces no lesson content from memory alone.
...
> `m16-orchestration`: workflows & `ultracode` · agent teams & cross-session messaging · designing a multi-agent pipeline (lab)

No lesson in this module exists yet; `content/en/l4-master/m16-orchestration/**` starts from the level/module `index.mdx` stub P06 created.
...
In:
- 3 lesson files under `content/en/l4-master/m16-orchestration/`, numbered `01-`…`03-`, each a complete MDX file following D006's seven sections.
- The matching Turkish draft stub for each lesson under `content/tr/l4-master/m16-orchestration/` (`draft: true`, created by the `/new-lesson` skill in the same commit as the EN lesson — D016, D018; P25/P26/P40/P41 translate it later, never this plan).
- One raw lab transcript per lesson with a hands-on lab, saved under `content/_shared/transcripts/m16-orchestration/`, plus the simplified `<Transcript>` rendering embedded in the lesson (D099).
...
- Updating `l4-master/index.mdx` and `m16-orchestration/index.mdx` module-index copy if this module's final lesson list differs from the stub P06 left (rare; only if `docs/CURRICULUM.md` changed the outline).

Out: Turkish translation of these lessons (P25/P26/P40/P41 own `content/tr/l4-master/m16-orchestration/**` after this plan hands it off); any other module's content; MDX component code (P07/P08 own those; if a lesson needs a component that does not exist, write it into `open_questions` and do not build it here); changes to `content/schema.ts`, `src/lib/nav.ts`, or any route/layout file; the lab repo itself (P22 owns `codechup/claude-code-lab`; this plan clones and runs it, never edits it in place — a bug fix needed there goes into `open_questions` naming the tag).
...
1. Read `docs/CURRICULUM.md` §m16-orchestration and confirm the lesson list/order below still matches; if it drifted, follow the doc, not this plan, and note the delta in Handoff notes.
...
3. Clone `codechup/claude-code-lab` (P22) into a scratch directory; for every lesson with a hands-on lab, check out its start tag (`lesson/m16-orchestration-NN-start`), do the exercise for real, and capture the full terminal transcript.
4. Write each lesson MDX with `/new-lesson l4-master/m16-orchestration/<slug>` (creates the EN file and the TR draft stub together); fill Objectives, When NOT to use, Concept (grounded in the researcher's summary, never invented), Hands-on lab (the exact commands from step 3), Anti-patterns, Quiz, Sources.
5. Save the raw transcript from step 3 under `content/_shared/transcripts/m16-orchestration/NN-<slug>.md`; embed the simplified version with the `<Transcript>` component (P07) in the lesson.
6. Append this lesson's sources to `content/_shared/sources.json` (P23's schema); run `/verify-sources content/en/l4-master/m16-orchestration/NN-<slug>.mdx` to stamp `verified_at`.
...
A reviewer opens `npm run dev`, visits each of the 3 lessons at `/en/l4-master/m16-orchestration/<slug>/`, confirms the OS tabs, transcript, quiz, and sources block render, spot-checks two Sources links by hand, and reads the fact-checker/reviewer agent reports pasted in the PR before merging.
