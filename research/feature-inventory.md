...
| Models | Fable 5.1 (1M ctx), Fable 5, Opus 5/4.8/4.7/4.6, Sonnet 5/4.6, Haiku 4.5; aliases `default best fable opus sonnet haiku sonnet[1m] opus[1m] opusplan`; ids `claude-fable-5-1`, `claude-fable-5`, `claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5`; Bedrock/Vertex/Foundry carry older sets; `modelPicker` curation | model-config (re-fetched 2026-09-06) |
| Effort / fast | `/effort low|medium|high(default)|xhigh|max`, `/effort ultracode` (a Claude Code setting that plans dynamic workflows), `/effort auto` clears; `max` is session-only unless `CLAUDE_CODE_EFFORT_LEVEL`; persisted per model in `modelSettings.<id>.effortLevel` or global `effortLevel`; `ultrathink` keyword for one turn; Opus 4.6/Sonnet 4.6 lack `xhigh`; `/fast` toggles to a lower effort for speed and refuses when it would switch to an Opus outside `availableModels` | model-config (re-fetched 2026-09-06), fast-mode |
